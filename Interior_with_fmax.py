...
def get_matrix():
    ...
    temp = [0.0] * (n + m + 1)
    matrix = [[0.0] * (n + m + 1) for _ in range(n + 1)]
    ...
    for i in range(1, n + 1):
        matrix[i][n + m] = get_with_accuracy(b[i - 1], accuracy)
...
"""
example of how the arrays should look like
...
example of input for this case
3
6
9 10 16 0 0 0
18 15 12 1 0 0
...
if not np.linalg.matrix_rank(A) == min(A.shape):
    print("The method is not applicable!")
    sys.exit(0)
print("enter initial x or -1 to generate the point automatically")
X = np.array(list(map(float, input().split())))
if len(X) == 1 and X[0] == -1:
    X = x = np.linalg.lstsq(A, b, rcond=None)[0]
    print()
    print("generated x: ", X)
    print()
x = X
